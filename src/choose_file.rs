...
fn update(
    ...
    std::fs::read_dir(cur_path)
        .unwrap()
        .skip(number_of_skips)
        .take(terminal_size.1 as usize - 2)
        .for_each(|path| {
            ...
            Ok(event) => match event {
                Event::Mouse(MouseEvent::Press(MouseButton::Left, x, y)) => {
                    if y == 1 {
                        ...
                        let vec = read_dir_into_sorted_vec(
                            &cur_dir
                                .to_owned()
                                .unwrap()
                                .into_os_string()
                        ...
                        let file = match vec
                            .iter()
                            .skip(number_of_skips + y as usize - 2)
                            .peekable()
                            .peek()
                        {
...
fn read_dir_into_sorted_vec(cur_dir: &String) -> Vec<DirEntry> {
    ...
    dirs.sort_by(|path, path2| {
        path.file_name()
            .into_string()
            .unwrap()
            .partial_cmp(&path2.file_name().into_string().unwrap())
            .unwrap()
    });
    files.sort_by(|path, path2| {
        path.file_name()
            .into_string()
            .unwrap()
            .partial_cmp(&path2.file_name().into_string().unwrap())
