...
impl Piece {
    pub fn get_string(&self, piece_table: &PieceTable) -> String {
        if self.buffer == Buffer::Add {
            piece_table.add[self.offset..self.offset + self.length]
                .into_iter()
                .collect::<String>()
        } else {
            piece_table.read[self.offset..self.offset + self.length]
                .into_iter()
                .collect::<String>()
        }
    }
...
impl PieceTable {
    ...
    pub fn insert(&mut self, idx: usize, text: String) {
        ...
        let piece_and_offset: Vec<usize> = self
            .get_piece_by_index(idx)
            .expect("I hope it won't happen");
        ...
        if cur_piece.buffer == Buffer::Add {
            if cur_piece.length + cur_piece.offset == self.add.len()
                && cur_piece.length == piece_and_offset[1]
            {
    ...
    pub fn remove(&mut self, idx: usize, length: usize) -> Option<String> {
        ...
        if start_piece_and_offset[0] == stop_piece_and_offset[0] {
            if start_piece_and_offset[1] == 0 {
                ...
                return Some(
                    text.chars().into_iter().collect::<Vec<char>>()[..length]
                        .into_iter()
                        .collect::<String>(),
                );
                ...
                self.pieces[start_piece_and_offset[0]].length -= length;
                return Some(
                    text.chars().into_iter().collect::<Vec<char>>()
                        [self.pieces[start_piece_and_offset[0]].length..]
                        .into_iter()
                        .collect::<String>(),
                );
            }
        ...
        for piece in self.pieces
            [start_piece_and_offset[0]..(start_piece_and_offset[0] + delete_cnt + 1)]
            .to_vec()
        {
            ...
            if piece == self.pieces[start_piece_and_offset[0]] {
                piece_str = piece
                    ...
                    .chars()
                    .into_iter()
                    .collect::<Vec<char>>()[start_piece_and_offset[1]..]
                    .into_iter()
                    .collect::<String>();
                ...
                piece_str = piece
                    ...
                    .chars()
                    .into_iter()
                    .collect::<Vec<char>>()[..stop_piece_and_offset[1]]
                    .into_iter()
                    .collect::<String>();
            } else {
            ...
            if deleted_str.chars().count() >= length {
                if deleted_str.chars().count() > length {
                    deleted_str = deleted_str.chars().into_iter().collect::<Vec<char>>()[..length]
                        .into_iter()
                        .collect::<String>();
                }
                break;
