...
impl TextManager {
    ...
    pub fn get_text_with_offset(&self) -> String {
        remove_prefix_and_update_lines(
            self.text.get_text(),
            self.offset.0,
            terminal_size().unwrap().0 as usize,
            self.offset.1,
            (terminal_size().unwrap().1 - 1) as usize,
        )
    }
    ...
    pub fn update_lines_lenghts(&mut self) {
        self.lines_manager
            .recalculate_line_lenghts(self.text.get_text());
    }
        ...
        if function.is_some() {
            ...
            match reversable_function.get_func() {
                ...
                Funcs::Remove => {
                    self.text.insert(
                        *reversable_function.get_index(),
                        reversable_function.get_string().to_owned(),
                    );
                }
                Funcs::Delete => {
                    self.text.insert(
            ...
            self.cursor
                .set_x_actual(reversable_function.get_cursor().get_x_actual());
            self.cursor
                .set_y_actual(reversable_function.get_cursor().get_y_actual());
            self.cursor
                .set_x_display(reversable_function.get_cursor().get_x_display());
            self.cursor
                .set_y_display(reversable_function.get_cursor().get_y_display());
        }
    }
    ...
    pub fn inc_x(&mut self) {
        if self.cursor.get_x_actual() == self.get_line_length(self.cursor.get_y_actual() - 1) + 1
            && self.cursor.get_y_actual() != self.get_num_lines()
        {
            self.inc_y();
            self.cursor.set_x_actual(1);
            self.cursor.set_x_display(1);
        } else if self.cursor.get_x_actual()
            != self.get_line_length(self.cursor.get_y_actual() - 1) + 1
        {
            self.cursor.inc_x();
        }
    ...
    pub fn inc_y(&mut self) {
        ...
        self.cursor.inc_y();
        self.cursor.set_x_actual(min(
            self.cursor.get_max(),
            self.get_line_length(self.cursor.get_y_actual() - 1) + 1,
        ));
        self.cursor.set_x_display(min(
            self.cursor.get_max(),
            self.get_line_length(self.cursor.get_y_actual() - 1) + 1,
        ) as u16);
    }
    ...
    pub fn dec_x(&mut self) {
        if self.cursor.get_x_actual() == 1 && self.cursor.get_y_actual() != 1 {
            self.dec_y();
            self.cursor
                .set_x_actual(self.get_line_length(self.cursor.get_y_actual() - 1) + 1);
            self.cursor
                .set_x_display((self.get_line_length(self.cursor.get_y_actual() - 1) + 1) as u16);
    ...
    pub fn update_offset(&mut self, old_cursor: &CursorPos) {
        ...
        if old_x < self.cursor.get_x_actual() {
            if self.cursor.get_x_actual() - old_x
                > (terminal_size.0 - self.cursor.get_x_display()) as usize
                && self.cursor.get_x_actual() > terminal_size.0 as usize
            {
        ...
        if old_y < self.cursor.get_y_actual() {
            if self.cursor.get_y_actual() - old_y
                > (terminal_size.1 - 1 - self.cursor.get_y_display()) as usize
            {
