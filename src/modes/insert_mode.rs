...
fn update(
    ...
    if coloring == (0, 0) {
        write!(
            stdout,
            "{}",
            document.get_text_with_offset().replace("\n", "\n\r")
        )
        .unwrap();
    } else {
    ...
    if terminal_size.1 >= 30 && terminal_size.0 >= 100 {
        ...
        write!(
            stdout,
            "{}",
            termion::cursor::Goto(terminal_size.0 - 30, terminal_size.1)
        )
        .unwrap();
        write!(
            stdout,
            "{} , {}",
            document.get_cursor().get_x_actual(),
            document.get_cursor().get_y_actual()
        )
        .unwrap();
        write!(
            stdout,
            "[{},{}]",
            document.get_offset().0,
            document.get_offset().1
        )
        .unwrap();
    ...
    write!(
        ...
        "{}",
        termion::cursor::Goto(
            document.get_cursor().get_x_display(),
            document.get_cursor().get_y_display()
        )
    )
    .unwrap();
    stdout.flush().unwrap();
    ...
    loop {
        if window_size != terminal_size().unwrap() {
            ...
            if highlight_start.is_some() && highlight_current.is_some() {
                let first_index = get_text_index_display(
                    document.get_text_with_offset(),
                    highlight_start.as_ref().unwrap(),
                );
                let second_index = get_text_index_display(
                    document.get_text_with_offset(),
                    highlight_current.as_ref().unwrap(),
                );
            ...
            Ok(event) => {
                match event {
                    ...
                    Event::Key(Key::Ctrl('c')) => {
                        if highlight_start.is_some() && highlight_current.is_some() {
                            let mut first_index = get_text_index_display(
                                document.get_all_text(),
                                highlight_start.as_ref().unwrap(),
                            );
                            let mut second_index = get_text_index_display(
                                document.get_all_text(),
                                highlight_current.as_ref().unwrap(),
                            );
                    ...
                    Event::Mouse(me) => match me {
                        ...
                        MouseEvent::Press(MouseButton::WheelDown, _x, _y) => {
                            ...
                            document.set_offset((
                                ...
                                min(
                                    document_offset.1 + 1,
                                    (document.get_num_lines() as isize
                                        - terminal_size().unwrap().1 as isize)
                                        .abs() as usize,
                                ),
                            ));
...
fn handle_backspace(
    document: &mut Document,
    highlight_start: Option<CursorPos>,
    highlight_current: Option<CursorPos>,
) {
    if highlight_start.is_some() && highlight_current.is_some() {
    ...
    document.push_to_undo_redo(Function::new(Funcs::Remove, idx, str, old_cursor));
}
...
fn remove_chunk(
    document: &mut Document,
    chunk_start: Option<CursorPos>,
    chunk_end: Option<CursorPos>,
) -> bool {
    ...
    let str = document.remove(
        idx,
        max(start_index, end_index) - min(start_index, end_index),
    );
...
fn print_text(
    ...
    let _ = text.chars().enumerate().for_each(|(i, c)| -> () {
    ...
    write!(
        stdout,
        "{}",
        until_first.iter().collect::<String>().replace("\n", "\n\r")
    )
    .unwrap();
    write!(
        stdout,
